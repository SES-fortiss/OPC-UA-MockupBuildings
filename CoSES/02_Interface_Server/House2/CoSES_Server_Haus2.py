...
print('price forecast gas boiler: ', priceGas_MEMAP_FC, ', for minutes', horizon_min_MEMAP)
...
server1.get_node('ns=2;i=121').set_value(11.2) #heat_demand
server1.get_node('ns=2;i=138').set_value(2.0) #electricity_demand
server1.get_node('ns=2;i=149').set_value(11.2) #boiler_power
...
while True:
    ...
    if k == 0: # first iteration in timestep
        ...
        server1.get_node('ns=2;i=149').set_value(demand1_CoSES[((l - 2) * time_ratio) + k] + 1.0)  # boiler_power for test
