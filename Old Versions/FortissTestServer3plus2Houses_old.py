...
(General, Demand, Producer, VolatilePruducer, Coupler, Storage) = create_Namespace(server1, idx, objects)
...
(B1_P_VProd1, B1_Eff_VProd1) = add_VolatileProducer(idx, Geb+"_Photovoltaik", VolatilePruducer, "Electricity", 0.18, 18, 0)

# Coupler - add_Coupler(idx, Geb, Coupler, Medium1, Medium2, Eff1, Eff2, P_min, P_max, Temp)
(B1_Pp_Coup1, B1_Eff1_Coup1, B1_Ps_Coup1, B1_Eff2_Coup1) = add_Coupler(idx,  Geb+"_Heatpump", Coupler, "Heat", "Electricity", 3.8, -1, 0, 10, 40)

# Storage - add_Storage(idx, name, Storage, Medium, Eff, Capacity, Pmax_in, Pmax_Out, Temp, SOC_alt)
(B1_Eff_Stor1, B1_Cap_Stor1, B1_P_in_Stor1, B1_P_out_Stor1, B1_SOC_Stor1) = add_Storage(idx, Geb+"_Battery", Storage, "Electricity", 0.98, 12, 3.3, 3.3, 0, 0.5)
...
(General, Demand, Producer, VolatilePruducer, Coupler, Storage) = create_Namespace(server2, idx, objects)
...
# VolatileProducer - add_VolatileProducer(idx, Geb, VolatilePruducer, Medium, Eff, Area, Temp):
(B2_P_VProd1, B2_Eff_VProd1) = add_VolatileProducer(idx, Geb+"_Solarthermic", VolatilePruducer, "Heat", 0.5, 3, 110)

# Coupler - add_Coupler(idx, Geb, Coupler, Medium1, Medium2, Eff1, Eff2, P_min, P_max, Temp)
(B2_Pp_Coup1, B2_Eff1_Coup1, B2_Ps_Coup1, B2_Eff2_Coup1) = add_Coupler(idx,  Geb+"_CHP", Coupler, "Heat", "Electricity", 0.6, 0.25, 0, 3.6, 80)

# Storage - add_Storage(idx, name, Storage, Medium, Eff, Capacity, Pmax_in, Pmax_Out, Temp, SOC_alt)
(B2_Eff_Stor1, B2_Cap_Stor1, B2_P_in_Stor1, B2_P_out_Stor1, B2_SOC_Stor1) = add_Storage(idx, Geb+"_ThermalStorage", Storage, "Heat", 0.98, 20, 5, 5, 80, 0.5)
